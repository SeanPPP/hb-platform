/* 精确回退：把 IX_sales_order_detail_OrderGuid 恢复为不含 Quantity 的原定义（在线重建，不删除索引）。 */
USE [POSM];
GO
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF DB_NAME()<>N'POSM' OR @@SERVERNAME<>N'10_3_8_3'
    THROW 54100, N'拒绝回退：数据库或 SQL 实例与 2026-09-19 基线不一致。', 1;
IF NOT EXISTS (SELECT 1 FROM sys.database_recovery_status WHERE database_id=DB_ID()
    ...
    THROW 54101, N'拒绝回退：database_guid 与基线不一致。', 1;
...
DECLARE @cols nvarchar(4000) = (
    ...
    WHERE i.object_id=OBJECT_ID(N'dbo.sales_order_detail') AND i.name=N'IX_sales_order_detail_OrderGuid' AND i.type=2);
IF @cols IS NULL OR @cols <> N'K:OrderGuid;I:ProductCode;I:OrderDetailGuid;I:Quantity'
    THROW 54102, N'拒绝回退：IX_sales_order_detail_OrderGuid 现定义不是本方案产生的定义。', 1;

CREATE NONCLUSTERED INDEX [IX_sales_order_detail_OrderGuid]
ON dbo.[sales_order_detail] ([OrderGuid])
INCLUDE ([ProductCode], [OrderDetailGuid])
