...
{
    internal const int MigrationIdMaxLength = 160;

    private static readonly string CreateHistoryTableSqlTemplate = $$"""
        IF OBJECT_ID(N'[dbo].[{0}]', N'U') IS NULL
        BEGIN
            CREATE TABLE [dbo].[{0}] (
                [MigrationId] nvarchar({{MigrationIdMaxLength}}) NOT NULL,
