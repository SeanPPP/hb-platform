...
public sealed class SchemaMigrationCoordinatorTests
{
    ...
    public async Task Coordinator_两个baseline维持既定顺序且成功后才登记()
    {
        ...
        Assert.True(
            mainCreateTable < mainStartup && mainStartup < mainApplicationLog,
            "主库 baseline 必须依次执行 CreateTable、StartupSchemaMigrator、ApplicationLogSchemaMigrator。"
        );
        ...
        Assert.True(
            IndexOfRequired(source, "RecordAppliedAsync")
                > IndexOfRequired(source, "await _runtime.ApplyBaselineAsync(database, cancellationToken)"),
    ...
    public async Task Coordinator_已应用跳过并支持主库成功后仅重跑POSM()
    {
        ...
        Assert.Contains("_runtime.ApplyBaselineAsync", runMigration, StringComparison.Ordinal);
        ...
        Assert.True(
            IndexOfRequired(runMigration, "SCHEMA_MIGRATION_ALREADY_APPLIED")
                < IndexOfRequired(runMigration, "_runtime.ApplyBaselineAsync"),
    ...
    [Fact]
    public async Task Coordinator_Check仅做三批只读门禁且不执行迁移器()
    {
        var source = await ReadCoordinatorSourceAsync();
        ...
        // 两个账本查询 + 一条 DeviceActivation VerifySql，启动门禁不能重新扫描/写入 schema。
    ...
    public async Task MigrateAsync_baseline失败不得写入该数据库账本()
    {
        ...
        Assert.Contains("Apply:Main", runtime.Events);
    ...
    public async Task MigrateAsync_POSM中途失败后重跑只继续POSM()
    {
        ...
        Assert.DoesNotContain("Apply:Main", runtime.Events);
        Assert.DoesNotContain("Record:Main:20260827.001-hbweb-baseline", runtime.Events);
        Assert.Contains("Apply:Posm", runtime.Events);
    ...
    [Fact]
    public async Task CheckAsync_缺少账本返回20且固定执行三批只读门禁()
    {
        var runtime = new FakeSchemaMigrationRuntime();
        runtime.MarkApplied(SchemaDatabase.Posm, SchemaMigrationCoordinator.PosmMigrationId);
        ...
        Assert.Equal(
            ["EnsureProviders", "Check:Main:20260827.001-hbweb-baseline", "Check:Posm:20260827.001-hbweb-posm-baseline", "Verify"],
            runtime.Events
        );
    ...
    private sealed class FakeSchemaMigrationRuntime : ISchemaMigrationRuntime
    {
        ...
        public Task ApplyBaselineAsync(SchemaDatabase database, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            Events.Add($"Apply:{database}");
