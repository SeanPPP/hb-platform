...
- 本次 DeviceCode 必须由当前测试机器独占，禁止另一台机器在同一时间复用；测试虽要求五类事件与登录事件的 InstanceId 完全一致，但 InstanceId 校验不能替代机器级独占前提
...
- `WpfAppFixture.cs`：启动/关闭 WPF、创建 UIA3 会话、执行 live 进程预检，并按敏感状态收集安全失败证据。
...
- 固定窗口内其他门店的正常并发审计也会被视为窗口污染并使测试失败；该失败仅说明证据不再排他，不代表测试修改了该门店。
...
- 失败证据始终报告测试步骤和客户端日志路径，并保留原始业务异常；仅在能够确认安全脱敏时才将窗口截图保存到 `%TEMP%\hbpos-ui-tests\{run-id}`，截图和日志均不提交到仓库。
- 安全截图前先清空收银员和商品输入；若无法确认清空成功，证据必须明确报告截图“未生成”，不得把脱敏或截图异常替换为原始业务异常。
- live 开始尝试输入收银员条码或商品条码等任一受保护值后，界面状态区可能回显输入，因此后续失败一律明确报告截图“未生成”并跳过整窗截图；支付成功页仍可安全截图。
- 后台完成采用有上限的轮询，达到超时后报告已观察到的事件集合。
- 清理阶段只关闭由测试启动的 WPF 进程，不终止用户已打开的其他实例。
- live 启动前只读检测同一目标 executable；发现既有进程或无法确认同名进程路径时立即失败，Preview 不受该门禁影响。
...
- 其他门店的前后只读快照无新增记录。
- 失败证据始终包含步骤和日志路径并保留原始异常；安全时包含已生成的截图路径，敏感值已提交或无法确认脱敏时明确报告截图“未生成”，且不泄露收银员凭据、设备授权码或支付信息。
