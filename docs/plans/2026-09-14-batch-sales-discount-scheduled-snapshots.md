...
批量销量页面从 HBweb 的 `BatchProductSalesDiscountSnapshot` 直接读取预计算结果。切换商品、日期范围、分店或刷新状态均不扫描 POS 原始交易，不创建折扣计算任务。独立后台任务持续维护近一年数据，服务重启后继续未完成日期。

验收以真实查询条件和数据库结果为准：已完成日的正价、折扣、未知数量与净销量一致，销售额与日统计按存储精度一致；一个日期缺失或异常不能屏蔽其他已完成日期；分店权限在读取服务端执行。刷新成功、任务启动和一年回填全部完成分别报告。
...
2. **后台调度与发布层**：按日领取持久化任务，遵守配置和运行时定时任务总开关，使用独立全局租约和日期 token 防止重复工作及旧执行器提交。只有数量、金额和数据版本核对通过后才发布。
...
2026-09-15 按用户要求将默认值和生产 compose 显式配置改为一年。按 Brisbane 业务日期，2026-09-15 对应 2025-09-15 至 2026-09-15（首尾均包含，共 366 个日历日）。此前已创建的范围外状态与快照保留，但不再被领取或巡检。

折扣 worker 不依赖通用调度器的 `ActiveInstanceId`；旧 API 可能被自动选为通用调度实例，却不包含新版折扣 worker，导致回填永久跳过。折扣任务继续遵守 `ScheduledTasks.Enabled` 和持久化 `SchedulerEnabled` 总开关，通过已有 `BatchProductSalesDiscountWorker/daily-format-2` 数据库租约和日期 token 控制多实例互斥。其他统计任务的选主行为保持原样。

- 以系统统一的业务日期为准，自动维护从当天向前一年的日期任务；未完成日期持久化保存，按日期分批处理。
...
- 每轮最多 12 个日期、总预算四分钟；仍有待办时五秒后续跑，空闲时每分钟检查。领取任务和待办判断在 SQL 中限定当前一年范围，过期历史数据保留但不继续巡检。
...
| GPT-5.6 Terra 调度代理 | DailyStore、Worker、调度注册、任务测试 | 一年任务、日租约、原子发布、重试与近期优先 |
