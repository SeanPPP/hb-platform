    ...
    public class ContainerMappingProfile : BaseMappingProfile
    {
        ...
        private void ConfigureContainerMappings()
        {
            ...
            CreateMap<CPT_RED_货柜单主表Store, ContainerMainDto>()
                .ForMember(dest => dest.Details, opt => opt.MapFrom(src => src.Details.Where(d => d.商品信息 != null)));
            ...
            CreateMap<CPT_DIC_商品信息字典表, ContainerProductInfoDto>()
                ...
                .ForMember(dest => dest.商品名称, opt => opt.MapFrom(src => src.中文名称 ?? src.英文名称)) // 优先显示英文翻译
                ...
                .ForMember(dest => dest.商品类型, opt => opt.MapFrom(src => src.商品类型.HasValue ? src.商品类型.Value == 0 ? "普通商品" : src.商品类型.Value == 1 ? "套装商品" : src.商品类型.Value == 2 ? "套装子商品" : "未知" : null))
            ...
            CreateMap<Container, ContainerMainDto>()
                ...
                .ForMember(dest => dest.预计到岸日期, opt => opt.MapFrom(src => src.EstimatedArrivalDate))
                .ForMember(dest => dest.实际到货日期, opt => opt.MapFrom(src => src.ActualArrivalDate))
                ...
                .ForMember(dest => dest.Details, opt => opt.MapFrom(src => src.Details.Where(d => d.Product != null)));
            ...
            CreateMap<ContainerDetail, ContainerDetailDto>()
                ...
                .ForMember(dest => dest.是否新商品, opt => opt.MapFrom(src => src.LocalProduct == null))
            ...
            CreateMap<DomesticProduct, ContainerProductInfoDto>()
                ...
                .ForMember(dest => dest.商品名称, opt => opt.MapFrom(src => src.ProductName ?? src.EnglishProductName)) // 优先显示英文名称
                ...
                .ForMember(dest => dest.商品规格, opt => opt.MapFrom(src => src.ProductSpecification))
                .ForMember(dest => dest.单位, opt => opt.Ignore()) // DomesticProduct 没有单位字段，忽略
                .ForMember(dest => dest.单件装箱数, opt => opt.MapFrom(src => src.PackingQuantity))
                .ForMember(dest => dest.单件体积, opt => opt.MapFrom(src => src.UnitVolume))
                .ForMember(dest => dest.商品类型, opt => opt.MapFrom(src => src.ProductType == 0 ? "普通商品" : src.ProductType == 1 ? "套装商品" : src.ProductType == 2 ? "套装子商品" : "未知"))
