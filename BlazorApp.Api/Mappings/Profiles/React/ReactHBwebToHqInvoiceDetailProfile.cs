    ...
    public class ReactHBwebToHqInvoiceDetailProfile : Profile
    {
        public ReactHBwebToHqInvoiceDetailProfile()
        {
            CreateMap<StoreLocalSupplierInvoiceDetails, RED_进货单详情表Store>()
                ...
                .ForMember(
                    dest => dest.H商品标签GUID,
                    opt => opt.MapFrom(src => src.ProductTagGUID)
                )
                .ForMember(
                    dest => dest.H商品分类码GUID,
                    opt => opt.MapFrom(src => src.ProductCategoryGUID)
                )
                .ForMember(dest => dest.H供应商编码, opt => opt.MapFrom(src => src.SupplierCode))
                .ForMember(
                    dest => dest.H分店商品编码,
                    opt => opt.MapFrom(src => src.StoreProductCode)
                )
                ...
                .ForMember(
                    dest => dest.H上次进货价,
                    opt => opt.MapFrom(src => src.LastPurchasePrice)
                )
                ...
                .ForMember(
                    dest => dest.H已存在商品数,
                    opt => opt.MapFrom(src => src.ExistingProductCount)
                )
                ...
                .ForMember(
                    dest => dest.H新自动零售价,
                    opt => opt.MapFrom(src => src.NewAutoRetailPrice)
                )
                .ForMember(
                    dest => dest.H是否特殊商品,
                    opt => opt.MapFrom(src => src.IsSpecialProduct)
                )
                .ForMember(
                    dest => dest.H老库分店商品编码,
                    opt => opt.MapFrom(src => src.OldStoreProductCode)
                )
