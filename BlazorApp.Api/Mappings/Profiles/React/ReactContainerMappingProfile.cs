    ...
    public class ReactContainerMappingProfile : Profile
    {
        public ReactContainerMappingProfile()
        {
            CreateMap<CPT_RED_货柜单主表Store, Container>()
                .ForMember(dest => dest.ContainerCode, opt => opt.MapFrom(src => TrimLen(src.HGUID, 50) ?? $"CONTAINER_{src.ID}"))
                .ForMember(dest => dest.ContainerNumber, opt => opt.MapFrom(src => TrimLen(src.货柜编号, 50)))
                ...
                .ForMember(dest => dest.Remarks2, opt => opt.MapFrom(src => TrimLen(src.备注2, 1000)));
        }
