...
import type { UpdateOperationLeasePort } from "@hb/pos-domain/core/contracts/update-operation-lease";
...
const UPDATE_TRANSITION_IN_PROGRESS = "UPDATE_TRANSITION_IN_PROGRESS";

class TestUpdateOperationLease implements UpdateOperationLeasePort {
  ...
  public async runOperation<T>(operation: () => T | Promise<T>): Promise<T> {
    if (this.transitionActive) {
      throw Object.assign(new Error("Update transition is in progress."), {
        code: UPDATE_TRANSITION_IN_PROGRESS,
      });
    }
    ...
    try {
      ...
      if (this.activeOperations === 0) {
        for (const waiter of this.operationWaiters) waiter();
        this.operationWaiters.clear();
      }
    }
  }

  public async runTransition<T>(operation: () => Promise<T>): Promise<T> {
    this.transitionActive = true;
    if (this.activeOperations > 0) {
      await new Promise<void>((resolve) => this.operationWaiters.add(resolve));
    }
    try {
      return await operation();
    } finally {
