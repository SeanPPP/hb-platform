...
public sealed class LinklyBackendTerminalClientTests
{
    ...
    public async Task PurchaseAsync_does_not_use_short_configured_timeout_before_linkly_business_wait()
    {
        ...
        try
        {
            ...
            await statusStarted.Task.WaitAsync(TestWaitTimeouts.Default);
            ...
            var result = await purchaseTask.WaitAsync(TestWaitTimeouts.Default);
            Assert.True(result.Approved);
        }
        finally
        {
            ...
            cancellation.Cancel();
            await purchaseTask.WaitAsync(TestWaitTimeouts.Default);
        }
    }
