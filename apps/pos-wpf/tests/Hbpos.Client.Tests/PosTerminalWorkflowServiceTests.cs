...
public sealed class PosTerminalWorkflowServiceTests
{
    ...
    public async Task Add_selected_item_remote_delete_keeps_local_cart_line()
    {
        ...
        var catalogRefresh = await catalogReloaded.Task.WaitAsync(TestWaitTimeouts.Default);
    ...
    public async Task Add_selected_item_finishes_mainline_before_remote_lookup_dispatch()
    {
        ...
        uiPriority.Release();
        await remoteLookupStarted.Task.WaitAsync(TestWaitTimeouts.Default);
    ...
    public async Task Add_selected_item_waits_for_ui_idle_again_before_remote_apply()
    {
        ...
        await uiPriority.SecondWaitEntered.WaitAsync(TestWaitTimeouts.Default);
        ...
        await catalogReloaded.Task.WaitAsync(TestWaitTimeouts.Default);
    ...
    public async Task Add_selected_item_remote_apply_runs_on_originating_synchronization_context()
    {
        ...
        thread.Start();
        var threadIds = await completion.Task.WaitAsync(TestWaitTimeouts.Default);
        Assert.True(thread.Join(TestWaitTimeouts.Default));
