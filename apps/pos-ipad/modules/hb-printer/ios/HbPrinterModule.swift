...
public final class HbPrinterModule: Module {
  private let bluetoothQueue = DispatchQueue(label: "com.hbweb.posipad.printer.ble")
  ...
  public func definition() -> ModuleDefinition {
    ...
    AsyncFunction("connect") { (peripheralId: String, timeoutMs: Int, promise: Promise) in
      self.bluetoothQueue.async {
        guard self.ensureBluetoothReady(promise) else { return }
        guard self.connectPromise == nil else {
        ...
        self.emitStatus(reason: "connecting")
        self.bluetoothDelegate.centralManager?.connect(peripheral, options: nil)
  ...
  fileprivate func handlePeripheralConnected(_ peripheral: CBPeripheral) {
    bluetoothQueue.async {
      guard peripheral.identifier.uuidString == self.connectedPeripheralId else { return }
  ...
  fileprivate func handleWriteWithoutResponseReady(_ peripheral: CBPeripheral) {
    bluetoothQueue.async { self.flushPendingChunks() }
  }
  ...
  private func startWrite(
    ...
    guard !data.isEmpty else {
      let result = resultPayload(operationId: operationId, state: "failed", message: "空打印命令被拒绝。")
    ...
    let timeout = DispatchWorkItem { [weak self] in
      // 超时后不重放任何字节：应用层必须把此操作保留为 unknown 并人工处理。
      self?.finishPendingOperation(state: "unknown", message: "BLE 写入超时，无法确认打印或开箱结果。")
    }
    pendingOperation = PendingOperation(id: operationId, kind: kind, promise: promise, timeout: timeout)
  ...
  private func failConnect(code: String, message: String) {
    guard connectPromise != nil || connectedPeripheral != nil else { return }
  ...
  private func disconnectInternal(
    ...
    connectTimeout = nil
    if let peripheral = connectedPeripheral { bluetoothDelegate.centralManager?.cancelPeripheralConnection(peripheral) }
    writeCharacteristic = nil
    writeType = nil
