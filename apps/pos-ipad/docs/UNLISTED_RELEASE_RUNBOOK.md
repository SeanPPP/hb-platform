...
- `PosIpad.Enabled=false`，仅试点门店出现在
  `PosIpad.EnabledStoreCodes`。
...
- 先保持 `PosIpad.Enabled=false` 验证设备注册、验证、目录下载和只读页面，再只对
  试点门店打开交易门禁。
...
1. 先将受影响门店从 `PosIpad.EnabledStoreCodes` 移除，或设置
   `PosIpad.Enabled=false`，阻止新交易。
...
4. Unlisted 链接只解决发现与分发，不代表业务授权；应用仍须通过设备注册、审批、
   启用状态、门店范围和收银员权限。
5. 将最终 App Store URL 写入 Hbpos.Api 的 iPad 更新策略，先保持试点门店范围，
   经变更审批后再逐店放开。
