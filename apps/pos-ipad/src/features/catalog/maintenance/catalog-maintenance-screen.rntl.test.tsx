...
describe("CatalogMaintenanceScreen", () => {
  it("显示 idle 与下载状态，并保持所有主操作至少 44pt", async () => {
    let release!: () => void;
    ...
    const screen = await render(<CatalogMaintenanceScreen presenter={presenter} />);

    expect(screen.getByTestId("catalog-maintenance-idle")).toBeTruthy();
    expect(screen.getByText(/旧目录仍可继续使用/)).toBeTruthy();
    expect(
      StyleSheet.flatten(screen.getByTestId("catalog-maintenance-refresh").props.style).minHeight,
    ...
    await waitFor(() => expect(screen.getByTestId("catalog-maintenance-downloading")).toBeTruthy());
    expect(screen.getByTestId("catalog-maintenance-refresh").props.accessibilityState).toEqual({ disabled: true });
    await act(async () => {
      release();
      await Promise.resolve();
    });
    await waitFor(() => expect(screen.getByTestId("catalog-maintenance-success")).toBeTruthy());
  });

  it("成功显示 snapshotId 和 itemCount，失败只显示稳定安全码", async () => {
    const successPort = new ScreenCatalogMaintenancePort();
    const successful = await render(<CatalogMaintenanceScreen presenter={createPresenter(successPort)} />);
    await act(async () => {
    ...
    const failed = await render(<CatalogMaintenanceScreen presenter={createPresenter(failedPort)} />);
    await act(async () => {
    ...
    expect(failed.getByText(/旧目录仍可继续使用/)).toBeTruthy();
  });

  it("可选返回按钮保留至少 44pt 触控目标", async () => {
    const onBack = jest.fn();
    const screen = await render(
      <CatalogMaintenanceScreen onBack={onBack} presenter={createPresenter(new ScreenCatalogMaintenancePort())} />,
    );
    const back = screen.getByTestId("catalog-maintenance-back");
    expect(StyleSheet.flatten(back.props.style).minHeight).toBeGreaterThanOrEqual(CATALOG_MAINTENANCE_MIN_TOUCH_TARGET);
