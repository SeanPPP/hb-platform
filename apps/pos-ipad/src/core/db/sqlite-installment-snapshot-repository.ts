  ...
  implements InstallmentSnapshotRepositoryPort
{
  ...
  public async upsertForStore(
    ...
    // 与 replace 相同：整批校验和加密先完成，坏页不能留下半批更新。
    const prepared: PreparedSnapshot[] = [];
    const seenGuids = new Set<string>();
    for (const candidate of snapshots) {
      const snapshot = validateSnapshot(candidate, store);
  ...
  public async listForStore(
    ...
    return Object.freeze(
      await Promise.all(rows.map((row) => this.readSnapshot(row))),
    );
  }
  ...
  private async readSnapshot(
    ...
    try {
      payload = parseSensitivePayload(
        JSON.parse(await this.encryptor.decrypt(ciphertext)),
      );
    } catch (error) {
      if (
    ...
    return Object.freeze({
      ...
      createdAtIso: persistedCanonicalIso(
        row.created_at_iso,
        "created at",
      ),
      ...
      updatedAtIso: persistedCanonicalIso(
        row.updated_at_iso,
        "updated at",
      ),
      note: payload.note,
