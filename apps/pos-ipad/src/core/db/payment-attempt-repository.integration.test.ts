...
test("真实 SQLite：支付尝试保留身份、Approved 仅匹配正确 tender 解锁，并且重开连接后仍然成立", async () => {
  ...
  try {
    ...
    // M44：真实 SQLite 必须先保存准确 tender 才允许 ACK marker；marker 是同一
    // attempt 的耐久状态，重开连接后仍能证明不该重放确认。
    await insertDraftOrder(reopened, "order-linkly-ack", 6, 500);
    const ackAttempt = payment({
      attemptId: "attempt-linkly-ack",
      idempotencyKey: "idempotency-linkly-ack",
      orderGuid: "order-linkly-ack",
    ...
    assert.equal(await marker.call(reopenedRepositories.payments, ackApproved, "2026-07-28T00:03:02.000Z"), false);

    await insertDraftOrder(reopened, "order-linkly-declined", 7, 500);
    const declined = payment({ attemptId: "attempt-linkly-declined", idempotencyKey: "idempotency-linkly-declined", orderGuid: "order-linkly-declined", provider: "linkly-cloud", providerEnvironment: "production", state: "Declined", updatedAtIso: "2026-07-28T00:04:00.000Z" });
    // 直接插入终态只为验证 SQL marker gate；真实流程仍由 PaymentAttemptService 先 CAS 持久化。
    await reopened.run("INSERT INTO payment_attempts (attempt_id,idempotency_key,order_guid,provider,operation,amount_cents,state,checkout_id,payment_id,session_id,txn_ref,rfn,created_at_iso,updated_at_iso,last_error_code,provider_environment) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [declined.attemptId, declined.idempotencyKey, declined.orderGuid, declined.provider, declined.operation, declined.amount.cents, declined.state, null, null, null, null, null, declined.createdAtIso, declined.updatedAtIso, null, declined.providerEnvironment ?? null]);
    assert.equal(await marker.call(reopenedRepositories.payments, declined, "2026-07-28T00:04:01.000Z"), true);

    await insertDraftOrder(reopened, "order-linkly-legacy", 8, 500);
