import {
  ...
  type HeldOrderDeleteRepositoryPort,
...
export class HeldOrdersOrchestrator {
  ...
  private async deleteOnce(
    ...
    const repository = this.options.repository as HeldOrderDeleteRepositoryPort;
    const authorized = await this.withRecallAuthorization<HeldOrderActionResult>(
      "delete",
      async () => {
        const normalizedHoldId = requiredId(() => holdId, "Hold id");
        let stage;
        try {
          stage = await repository.stageDeletePending({
            holdId: normalizedHoldId,
            scope: heldOrderScope(this.options.identity),
        ...
        try {
          const deleted = await repository.deleteStagedPending({
            holdId: normalizedHoldId,
            scope: heldOrderScope(this.options.identity),
          });
          return deleted
