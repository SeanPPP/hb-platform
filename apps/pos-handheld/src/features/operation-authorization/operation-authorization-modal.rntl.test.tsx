...
import { operationAuthorizationText } from "./operation-authorization-copy";
import {
...
test("仅在待授权时显示，中英双语且输入与按钮满足安全和触控约束", async () => {
  ...
  expect(OPERATION_AUTHORIZATION_MIN_TOUCH_TARGET).toBe(48);
  expect(
    screen.getByTestId("handheld-state-operation-authorization"),
  ).toBeTruthy();
...
test("主管身份失败文案使用手持终端命名，不泄漏 iPad 产品名", () => {
  expect(operationAuthorizationText("en", "identityMismatch")).toContain(
    "device",
  );
  expect(operationAuthorizationText("zh", "identityMismatch")).toContain(
    "设备",
  );
  expect(
    `${operationAuthorizationText("en", "identityMismatch")} ${operationAuthorizationText("zh", "identityMismatch")}`,
  ).not.toMatch(/ipad/iu);
});
