...
export type AppUpdateRecoveryStatus = Readonly<{
  payment: "clear" | "recovery-required";
  sync: "clear" | "in-progress";
}>;
...
export type AppUpdateRecoveryScreenState =
  ...
  | Readonly<{
      kind: "ready";
      recovery: AppUpdateRecoveryStatus;
...
const copy = {
  en: {
    ...
    syncInProgress: "Sync and audit in progress",
    syncClear: "Sync and audit clear",
    paymentRecoveryRequired: "Payment recovery required",
    paymentClear: "No payment recovery pending",
    labels: {
  ...
  zh: {
    ...
    syncInProgress: "同步与审计正在处理",
    syncClear: "同步与审计已完成",
    paymentRecoveryRequired: "需要完成支付恢复",
    paymentClear: "没有待处理的支付恢复",
    labels: {
      appVersion: "应用版本",
      buildNumber: "构建号",
      runtimeVersion: "Runtime",
...
export function AppUpdateRecoveryScreen({
  ...
  return (
    ...
    >
      <HandheldStateSurface slug="update-recovery" style={styles.stateSurface}>
                ...
                <View style={styles.recoveryStatusCard}>
                  <View style={styles.recoveryStatusRow}>
                    <View
                      style={[
                        ...
                        state.recovery.sync === "in-progress"
                          ? styles.recoveryStatusWarning
                          : styles.recoveryStatusClear,
                      ]}
                    />
                    <Text
                      style={styles.recoveryStatusText}
                      testID="app-update-recovery-sync-state"
                    >
                      {state.recovery.sync === "in-progress"
                        ? text.syncInProgress
                        : text.syncClear}
                  ...
                  <View style={styles.recoveryStatusRow}>
                    <View
                      style={[
                        styles.recoveryStatusDot,
                        state.recovery.payment === "recovery-required"
                          ? styles.recoveryStatusDanger
                          : styles.recoveryStatusClear,
                      ]}
                    />
                    <Text
                      style={styles.recoveryStatusText}
                      testID="app-update-recovery-payment-state"
                    >
                      {state.recovery.payment === "recovery-required"
                        ? text.paymentRecoveryRequired
                        : text.paymentClear}
                    </Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  recoveryStatusDot: { borderRadius: 4, height: 8, width: 8 },
  recoveryStatusClear: { backgroundColor: posColors.green },
  recoveryStatusWarning: { backgroundColor: posColors.orange },
  recoveryStatusDanger: { backgroundColor: posColors.red },
  recoveryStatusText: {
    color: posColors.ink,
    flex: 1,
    fontSize: 14,
