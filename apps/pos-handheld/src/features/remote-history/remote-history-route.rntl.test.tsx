...
test("设备身份复核期间网络恢复时以最新状态创建 presenter 且不重建", async () => {
  mockShellStore.setState({ connectivity: "offline" });
  let resolveIdentity!: (
    identity: Readonly<{ storeCode: string; deviceCode: string }>,
  ) => void;
  mockRuntime = readyRuntime();
  mockRuntime.services.deviceSession.getDeviceIdentity = jest.fn(
    () =>
      new Promise((resolve) => {
  ...
  await act(async () => {
    mockShellStore.setState({ connectivity: "online" });
    resolveIdentity({ storeCode: "S1", deviceCode: "IPAD-1" });
