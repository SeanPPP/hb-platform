...
export const ATTENDANCE_AUDIT_MIN_TOUCH_TARGET = 48;
...
/**
 * 手持端将考勤 QR 与审计列表拆成独立单列状态；审计读取按 Audit.View
...
export function AttendanceAuditScreen({
  ...
  const [view, setView] = useState<"qr" | "audit">("qr");
  ...
  return (
    <SafeAreaView style={styles.safeArea} testID="attendance-audit-screen">
      <View style={styles.page}>
        ...
        />
        {view === "qr" ? (
          <HandheldStateSurface slug="attendance-qr" style={styles.stateSurface}>
            <ScrollView
              contentContainerStyle={styles.singlePaneScroll}
          ...
          <HandheldStateSurface slug="attendance-audit" style={styles.stateSurface}>
            <AuditWorkspace
              detailVisible={auditDetailVisible}
              onBackToList={() => setAuditDetailVisible(false)}
              onOpenDetail={() => setAuditDetailVisible(true)}
              presenter={presenter}
              state={state}
              t={t}
            />
    ...
    <View style={styles.header}>
      ...
      <View style={styles.headerActions}>
        ...
        <ActionButton
          ...
          selected={view === "qr"}
          sound="navigate"
          testID="attendance-show-qr"
          tone="secondary"
        />
        <ActionButton
          label={t("action.showAudit")}
          onPress={onShowAudit}
          selected={view === "audit"}
          sound="navigate"
          testID="attendance-show-audit"
          tone="secondary"
        />
      </View>
    </View>
  ...
  return (
    <View style={[styles.panel, styles.auditPanel]}>
      ...
      <View style={styles.auditMasterDetail}>
        {detailVisible ? (
          <View style={styles.auditDetailScreen}>
            <ActionButton
              label={t("action.backToAuditList")}
              onPress={onBackToList}
              sound="navigate"
              testID="audit-detail-back"
              tone="quiet"
            />
        ...
        {audit.rows.length === 0 ? (
          ...
          audit.rows.map((row) => (
            <AuditRow
              ...
              onPress={() => {
                onOpenDetail();
                void presenter.selectAudit(row.eventId);
              }}
              record={row}
