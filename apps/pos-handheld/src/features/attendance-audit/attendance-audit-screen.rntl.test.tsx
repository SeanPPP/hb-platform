...
describe("AttendanceAuditScreen", () => {
  ...
  it("考勤 QR 与审计列表是两个单列状态并可由顶部 48px 操作切换", async () => {
    const presenter = new ScreenPresenter();
    const screen = await render(
      <AttendanceAuditScreen presenter={presenter} />,
    );

    expect(screen.getByTestId("handheld-state-attendance-qr")).toBeTruthy();
    expect(screen.queryByTestId("handheld-state-attendance-audit")).toBeNull();
    const showAudit = screen.getByTestId("attendance-show-audit");
    expect(StyleSheet.flatten(showAudit.props.style).minHeight).toBeGreaterThanOrEqual(48);
    await fireEvent.press(showAudit);

    expect(screen.getByTestId("handheld-state-attendance-audit")).toBeTruthy();
    expect(screen.queryByTestId("handheld-state-attendance-qr")).toBeNull();
    expect(
      StyleSheet.flatten(screen.getByTestId("attendance-show-qr").props.style)
        .minHeight,
    ).toBeGreaterThanOrEqual(48);
  });

  it("单列 QR 与审计列表保留短时码、筛选回调和至少 48px 触控目标", async () => {
    const presenter = new ScreenPresenter();
    ...
    await fireEvent.press(screen.getByTestId("attendance-show-audit"));
  ...
  it("没有 Audit.View 仅保护审计区，不阻止考勤 QR", async () => {
    ...
    expect(screen.queryByTestId("attendance-qr-image")).toBeNull();
    await fireEvent.press(screen.getByTestId("attendance-show-qr"));
    expect(screen.getByTestId("attendance-qr-image")).toBeTruthy();
    await screen.unmount();
  });
  ...
  it("审计筛选、选择与详情只展示 presenter 的脱敏字段", async () => {
    ...
    expect(screen.getByTestId("audit-detail-back")).toBeTruthy();
    expect(
