...
export const SYNC_HISTORY_MIN_TOUCH_TARGET = 48;
...
export function SyncHistoryScreen({
  ...
  >("idle");
  const [selectedDetailGuid, setSelectedDetailGuid] = useState<string | null>(
    null,
  );
  ...
  const selectedDetail = selectedDetailGuid
    ? state.rows.find((row) => row.orderGuid === selectedDetailGuid) ?? null
    : null;
  ...
  return (
    <SafeAreaView style={styles.safeArea} testID="sync-history-screen">
      ...
      {selectedDetail ? (
        <HandheldStateSurface slug="sync-detail" style={styles.stateSurface}>
          <SyncHistoryDetail
            canRetransmit={canManualRetransmit}
            loading={loading}
            locale={locale}
            onBack={() => setSelectedDetailGuid(null)}
            onRetransmit={() => {
              if (!selectedDetail.isSelected) {
                presenter.setSelected(selectedDetail.orderGuid, true);
              }
        ...
        <HandheldStateSurface slug="sync-history" style={styles.stateSurface}>
                ...
                <SyncHistoryOrderRow
                  ...
                  onOpen={() => {
                    if (
                      ...
                      presenter.setSelected(item.orderGuid, true);
                    }
                    setSelectedDetailGuid(item.orderGuid);
                  }}
                  row={item}
                />
              )}
...
function SyncHistoryOrderRow({
  ...
  return (
    ...
    >
      <View
        accessibilityLabel={t("selection.count", { count: row.isSelected ? 1 : 0 })}
        accessibilityState={{ checked: row.isSelected, disabled: !canSelect }}
        style={styles.checkbox}
        testID={`sync-history-select-${row.orderGuid}`}
      >
...
function SyncHistoryDetail({
  ...
  const gateKey = row.retransmit.kind === "blocked"
    ...
    : null;
  const retryAllowed =
    canRetransmit && row.retransmit.kind === "allowed" && !loading;

  return (
    <ScrollView
      contentContainerStyle={styles.detailContent}
      testID="sync-history-detail-scroll"
    >
      <ActionButton
        label={t("action.backToList")}
        onPress={onBack}
        sound="navigate"
        testID="sync-history-detail-back"
        tone="quiet"
      />
      <View style={styles.detailSection}>
        <View style={styles.orderTitleRow}>
          <Text style={styles.detailSequence}>
            {t("list.sequence", { sequence: row.localSequence })}
          </Text>
          <StatusBadge locale={locale} state={row.state} />
        </View>
        <Text style={styles.orderGuid}>
          {t("list.order", { orderGuid: row.orderGuid })}
        </Text>
        <Text style={styles.orderTime}>{formatDateTime(row.soldAtIso, locale)}</Text>
        <Text style={styles.deviceLine}>{row.storeCode} · {row.deviceCode}</Text>
      </View>

      <View style={styles.detailSection}>
        <Text style={styles.detailLabel}>{t("list.amount")}</Text>
        <Text style={styles.detailAmount}>{formatAud(row.actualAmountCents, locale)}</Text>
      ...
      <View style={styles.detailSection}>
        <Text style={styles.detailLabel}>{t("list.outbox")}</Text>
        <Text style={styles.detailValue}>
          {row.outbox ? t(`outbox.${row.outbox.state}`) : "—"}
        </Text>
        {gateKey ? <Text style={styles.gateText}>{t(gateKey)}</Text> : null}
        {row.outbox ? (
          <Text style={styles.outboxMeta}>
            {t("list.attempts", { count: row.outbox.attemptCount })}
          </Text>
      ...
      <ActionButton
        ...
        label={t("action.retransmitSelected")}
        onPress={onRetransmit}
        testID="sync-history-detail-retransmit"
      />
    </ScrollView>
  );
}
