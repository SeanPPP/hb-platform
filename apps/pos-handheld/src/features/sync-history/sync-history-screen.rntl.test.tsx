...
describe("SyncHistoryScreen", () => {
  it("同步列表进入单列详情，详情保留原因与 48px 补传操作", async () => {
    ...
    expect(screen.getByTestId("handheld-state-sync-history")).toBeTruthy();
    expect(screen.queryByTestId("handheld-state-sync-detail")).toBeNull();
    await fireEvent.press(row);

    expect(screen.getByTestId("handheld-state-sync-detail")).toBeTruthy();
    expect(screen.getByText("Safe error: SYNC_NETWORK")).toBeTruthy();
    for (const testID of [
      "sync-history-detail-back",
      "sync-history-detail-retransmit",
    ]) {
      expect(
  ...
  it("选择订单后只把 eligible pending outbox 提交给补传 Port", async () => {
    ...
    expect(
      screen.getByTestId("sync-history-detail-retransmit").props
        .accessibilityState.disabled,
    ).toBe(false);
    expect(presenter.getState().selectedOrderGuids).toEqual(["eligible"]);
    await fireEvent.press(screen.getByTestId("sync-history-detail-retransmit"));

    await waitFor(() => {
      expect(port.restoreCalls).toEqual([["eligible"]]);
    });
    await fireEvent.press(screen.getByTestId("sync-history-detail-back"));
    expect(screen.getByTestId("sync-history-retransmit-result")).toBeTruthy();
  });
