...
test("任意视口采用单列列表进入详情，主要触控区不少于 48px", async () => {
  ...
  expect(screen.getByTestId("handheld-state-installments-list")).toBeTruthy();
  ...
  expect(screen.getByTestId("handheld-state-installment-detail")).toBeTruthy();
  ...
  expect(
    StyleSheet.flatten(screen.getByTestId("installments-details-back").props.style)
      .minHeight,
  ).toBe(INSTALLMENTS_MIN_TOUCH_TARGET);
  await fireEvent.press(screen.getByTestId("installments-details-back"));
  expect(screen.getByTestId("handheld-state-installments-list")).toBeTruthy();
  await screen.unmount();
});

test("详情选择被离线失效后，恢复在线回到列表而不是空详情", async () => {
  const created = createPresenter({
    details: details("Active"),
    orders: [summary()],
    selectedGuid: GUID,
  });
  const screen = await render(
    <InstallmentScreen presenter={created.presenter} />,
  );
  expect(screen.getByTestId("handheld-state-installment-detail")).toBeTruthy();

  await act(async () => {
    created.publish({
      ...created.presenter.getState(),
      details: null,
      online: false,
      orders: [],
      selectedGuid: null,
  ...
  expect(screen.getByTestId("handheld-state-installments-list")).toBeTruthy();
  expect(screen.getByTestId("installments-history-pane")).toBeTruthy();
  expect(screen.queryByTestId("installments-details-pane")).toBeNull();
  await screen.unmount();
});
...
test("详情加载和服务失败可重试，键盘避让合同保持启用", async () => {
  ...
  await fireEvent.press(loadingScreen.getByTestId(`installment-row-${GUID}`));
  expect(loadingScreen.getByTestId("installments-details-loading")).toBeTruthy();
...
test("已完成与已取消详情仍可在标题工具区重打，按钮满足 48px 与无障碍合同", async () => {
  for (const status of ["PickedUp", "Cancelled"] as const) {
    const { presenter, spies } = createPresenter(
      {
        details: details(status),
...
function createPresenter(
  ...
  let state: InstallmentPresenterState = {
    access: {
      canAddRepayment: true,
      canCancel: true,
  ...
  const publish = (nextState: InstallmentPresenterState): void => {
    state = nextState;
  ...
  const spies = {
    ...
    select: jest.fn(async (installmentGuid: string) => {
      publish({ ...state, selectedGuid: installmentGuid });
    }),
