...
describe("SpecialProductsScreen", () => {
  ...
  it("手持端使用两列商品网格并在受控编辑 modal 标记两个设计状态", async () => {
    ...
    expect(screen.getByTestId("handheld-state-special-products-grid")).toBeTruthy();
    expect(SPECIAL_PRODUCTS_GRID_COLUMNS).toBe(2);
    expect(
      StyleSheet.flatten(
    ...
    expect(screen.getByTestId("handheld-state-special-product-editor")).toBeTruthy();
    expect(
      StyleSheet.flatten(screen.getByTestId("special-products-add-modal-panel").props.style)
        .width,
    ).toBe("100%");
    expect(
      StyleSheet.flatten(screen.getByTestId("special-products-add-modal-close").props.style)
        .minHeight,
    ).toBeGreaterThanOrEqual(48);
  });

  it("离线显示本地两列网格并允许加购，但所有管理写操作禁用且触控至少 48px", async () => {
    const presenter = new ScreenPresenter({
      items: [product("A"), product("B")],
      online: false,
    });
    ...
    // 小屏固定两列并保留 8px 间距，避免 320px 视口横向溢出。
    expect(
      StyleSheet.flatten(
        screen.getByTestId("special-product-card-A-shell").props.style,
      ).width,
