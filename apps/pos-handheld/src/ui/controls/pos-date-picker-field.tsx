...
export function PosDatePickerField({
  ...
  // 弹层和内联日历只使用这一组窗口宽度计算，避免百分比上限再次压缩 320dp 面板。
  const panelWidth = Math.min(Math.max(windowWidth - 32, 0), 420);
  const pickerWidth = Math.min(Math.max(panelWidth - 34, 0), 360);
      ...
      {open ? (
        ...
        >
          ...
          >
            ...
            />
            <View style={[styles.panel, { width: panelWidth }]} testID={`${testID}-panel`}>
