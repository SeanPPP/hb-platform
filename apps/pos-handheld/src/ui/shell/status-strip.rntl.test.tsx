...
test.each([320, 375])(
  ...
  async (width) => {
    const screen = await render(
      <View style={{ width }}>
        <PosStatusStrip language="zh" onSwitchLanguage={jest.fn()} />
      </View>,
    );

    const containerStyle = StyleSheet.flatten(
      screen.getByTestId("status-strip").props.style,
    );
    expect(containerStyle).not.toHaveProperty("width");
    expect(containerStyle.position).not.toBe("absolute");

    const scroll = screen.getByTestId("status-strip-scroll");
    ...
    expect(contentStyle.flexWrap).not.toBe("wrap");

    for (const id of ["device", "network", "sync", "printer", "scanner"]) {
      expect(
        StyleSheet.flatten(
          screen.getByTestId(`status-strip-indicator-${id}`).props.style,
        ).flexShrink,
      ).toBe(0);
      expect(
