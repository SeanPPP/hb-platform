...
test("安装 HardwareId 在重复读取时稳定且空白生成值 fail-closed", async () => {
  ...
  const installation = new InstallationIdentityStore(secureStore, () => {
    generated += 1;
    return "INSTALL-STABLE-001";
  });

  assert.equal(await installation.getOrCreate(), "INSTALL-STABLE-001");
  assert.equal(await installation.getOrCreate(), "INSTALL-STABLE-001");
  ...
  await assert.rejects(
    ...
    /secure installation identifier/i,
  );
});
...
test("设备凭据仅接受四个完整非空字段，损坏或旧格式 Keychain 数据必须失败关闭", async () => {
  ...
  for (const raw of [
    ...
    await assert.rejects(
      credentials.load(),
      /Stored hbpos\.handheld\.device-credentials\.v1 is invalid/,
    );
  }
});
...
test("设备展示缓存使用独立 v1 Key 保存脱敏名称且仅限本机", async () => {
  ...
  assert.equal(
    await secureStore.get("hbpos.handheld.device-presentation.v1"),
    JSON.stringify({
      version: 1,
      deviceCode: "POS-1",
      storeCode: "S1",
  ...
  assert.equal(
    await secureStore.get("hbpos.handheld.device-credentials.v1"),
    null,
  );
  assert.deepEqual(secureStore.lastWriteOptions, localOnly);
});
...
test("损坏的设备展示缓存按无缓存处理并最佳努力清理", async () => {
  const key = "hbpos.handheld.device-presentation.v1";
  const secureStore = new InMemorySecureStore();
  const presentation = new DevicePresentationStore(secureStore);
...
test("待审批记录缺少设备或门店时不得进入本地 pending 状态", async () => {
  ...
  await secureStore.set(
    "hbpos.handheld.pending-device-registration.v1",
  ...
  await assert.rejects(
    pending.load(),
    /Stored hbpos\.handheld\.pending-device-registration\.v1 is invalid/,
  );
});
...
test("收银员离线缓存拒绝损坏字段和换绑内容，不把 JSON 类型断言当作认证", async () => {
  ...
  const key = `hbpos.handheld.cashier.v2.${"b".repeat(64)}`;
...
test("收银员缓存 Key 使用规范化三元组的 SHA-256，绝不把条码写入 Keychain 项名", async () => {
  ...
  assert.equal(secureStore.lastWriteKey, `hbpos.handheld.cashier.v2.${"c".repeat(64)}`);
  assert.deepEqual(secureStore.lastWriteOptions, localOnly);
});
...
test("收银员 bearer 活动期间可读，到期后立即 fail-close 并后台清除 Keychain", async () => {
  ...
  assert.equal(
    await secureStore.get(
      "hbpos.handheld.active-cashier-authorization.v1",
    ),
    null,
  );
});
...
test("收银员 bearer 必须精确匹配已验证的门店与设备 scope，旧 v2 envelope 不得猜测 scope", async () => {
  ...
  const key = "hbpos.handheld.active-cashier-authorization.v1";
