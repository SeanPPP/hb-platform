...
export default function FlowTrendChart({ data, ariaLabel }: FlowTrendChartProps) {
  ...
  return (
    <svg role="img" tabIndex={0} aria-label={ariaLabel} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="product-flow-chart">
      ...
      <desc>{t('productFlowShared.chartDescription')}</desc>
      <line x1="42" x2="704" y1={model.zeroY} y2={model.zeroY} stroke="#9aa4b2" strokeDasharray="4 4" />
      {model.series.inbound.map((bar, index) => <rect key={`inbound-${index}`} {...bar} fill="#246bfd" rx="1"><title>{t('productFlowShared.inboundTooltip', { value: bar.value })}</title></rect>)}
      {model.series.shipped.map((bar, index) => <rect key={`shipped-${index}`} {...bar} fill="#14b8a6" rx="1"><title>{t('productFlowShared.shippedTooltip', { value: bar.value })}</title></rect>)}
      {model.series.netSales.map((bar, index) => <rect key={`sales-${index}`} {...bar} fill={bar.value >= 0 ? '#f97316' : '#ef4444'} rx="1"><title>{t('productFlowShared.netSalesTooltip', { value: bar.value })}</title></rect>)}
      {pricePoints ? <polyline points={pricePoints} fill="none" stroke="#475569" strokeWidth="2" /> : null}
      {model.series.averageUnitPrice.map((point) => <circle key={`price-${point.date}`} cx={point.x} cy={point.y} r="3" fill="#475569"><title>{t('productFlowShared.averagePriceTooltip', { date: point.date, value: point.value })}</title></circle>)}
