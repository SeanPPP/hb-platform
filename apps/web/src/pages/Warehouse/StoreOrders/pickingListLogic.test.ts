...
runTest('配货单打印行高和字体应控制在每页约 30 行', () => {
  ...
  assertEqual(/text-align:\s*right/.test(indexRule), true, '三位行号应右对齐避免覆盖货号')
...
runTest('配货单打印列宽应按草图重新分配并保留 colgroup', () => {
  ...
  assertEqual(readCssWidth(readCssRule(printCssSource, '.store-order-picking-table .col-index')), 5.5, '行号列应为 5.5%')
