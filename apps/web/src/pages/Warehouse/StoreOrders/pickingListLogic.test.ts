...
runTest('配货单打印行高和字体应按 9mm 明细行稳定分页', () => {
  ...
  assertEqual(/background:\s*#fafafa/.test(sendQuantityRule), false, '发货数列不应固定背景色')
  assertEqual(/border-left:\s*1px/.test(sendQuantityRule), false, '发货数列不应覆盖统一单元格线')
...
runTest('配货单打印列宽应按草图重新分配并保留 colgroup', () => {
  ...
  assertEqual(readCssWidth(readCssRule(printCssSource, '.store-order-picking-table .col-qty')), 6.5, '订货数量列应为 6.5%')
  assertEqual(readCssWidth(readCssRule(printCssSource, '.store-order-picking-table .col-send-qty')), 7, '发货数列应为 7%')
...
runTest('配货单打印货号货位应保留间隔且名称价格区域更紧凑', () => {
  ...
  assertEqual(readCssWidth(quantityRule), 6.5, '订货数量列应为 6.5%')
  assertEqual(readCssWidth(sendQuantityRule), 7, '发货数列应为 7%')
...
runTest('配货单打印短表头应包含中英文翻译', () => {
  ...
  assertEqual(enLocale.warehouse.pickingList.innerPackShort, 'INNER QTY', '英文包数短表头应显示 INNER QTY')
