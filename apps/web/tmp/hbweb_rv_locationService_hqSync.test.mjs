...
function assertSyncSuccess(response, fallbackMessage) {
...
async function syncLocationsFromHq() {
  const locationResponse = await request_default.post(
    "/api/react/v1/sync/locations-incremental",
    {}
  );
  const locationResult = assertSyncSuccess(locationResponse, "\u4ECEHQ\u540C\u6B65\u8D27\u4F4D\u5931\u8D25");
  const productLocationResponse = await request_default.post(
...
// src/services/locationService.hqSync.test.ts
