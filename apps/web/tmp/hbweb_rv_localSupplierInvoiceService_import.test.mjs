...
async function updateLastPurchasePrices(invoiceGuid, data) {
...
async function previewInvoiceImport(file) {
...
async function confirmInvoiceImport(payload) {
  const response = await request_default.post(
...
// src/services/localSupplierInvoiceService.import.test.ts
...
var previewRequestUrl = "";
var previewRequestMethod = "";
var previewRequestBody;
var previewRequestHeaders;
var previewRequestCount = 0;
var confirmRequestUrl = "";
var confirmRequestMethod = "";
var confirmRequestBody = null;
var updateLastPurchaseRequestUrl = "";
var updateLastPurchaseRequestMethod = "";
var updateLastPurchaseRequestBody = null;
var refreshRequestCount = 0;
var previewResponses = [];
var testFile = new Blob(["demo"], {
...
globalThis.fetch = async (input, init) => {
  ...
  if (url.endsWith("/api/react/v1/local-supplier-invoices/import/confirm")) {
    ...
    confirmRequestBody = JSON.parse(String(init?.body || "{}"));
    return new Response(JSON.stringify({
      success: true,
      data: {
  ...
  if (url.endsWith("/api/react/v1/local-supplier-invoices/invoice-guid-001/details/update-last-purchase-prices")) {
    ...
    updateLastPurchaseRequestBody = JSON.parse(String(init?.body || "{}"));
    return new Response(JSON.stringify({
      success: true,
      data: {
        total: 2,
...
try {
  ...
  const previewResult = await previewInvoiceImport(testFile);
  ...
  assertEqual(
    previewRequestBody.get("file")?.size,
    testFile.size,
    "FormData \u4E2D\u5E94\u5305\u542B\u4E0A\u4F20\u6587\u4EF6"
  );
  assertEqual(
  ...
  previewRequestBody = void 0;
  previewRequestHeaders = void 0;
  previewResponses = [
  ...
  const retryPreviewResult = await previewInvoiceImport(testFile);
  ...
  try {
    await previewInvoiceImport(testFile);
