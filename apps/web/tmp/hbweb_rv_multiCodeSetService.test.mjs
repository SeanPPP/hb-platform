...
// src/services/multiCodeSetService.ts
var API_BASE = "/api/react/v1/product-set-codes";
async function getGridData(params) {
  const pageSize = params.pageSize ?? 200;
  const pageIndex = Math.max(1, params.pageIndex ?? 1);
  const startRow = (pageIndex - 1) * pageSize;
  const response = await request_default.post(`${API_BASE}/grid`, {
  ...
  const items = (result.items ?? []).filter((item) => item.productCode === params.productCode).map((item) => ({
    ...item,
...
// src/services/multiCodeSetService.test.ts
