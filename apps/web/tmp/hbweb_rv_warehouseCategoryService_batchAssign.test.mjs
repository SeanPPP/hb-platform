...
// src/services/warehouseCategoryService.ts
function ensureReactSuccess(payload, fallbackMessage) {
...
function extractAffectedCount(payload) {
  ...
  const affected = data.affected ?? data.Affected;
  return typeof affected === "number" && Number.isFinite(affected) ? affected : 0;
}
async function batchAssignProducts(categoryGuid, productCodes) {
...
// src/services/warehouseCategoryService.batchAssign.test.ts
