...
async function request(url, options = {}) {
  ...
  if (!response.ok) {
    if (response.status === 401 && !skipAuthRedirect && !AUTH_WHITELIST.has(normalizedUrl)) {
      ...
      if (refreshed) {
        const retryResult2 = await rawFetch(url, options);
        if (retryResult2.response.ok) {
          return retryResult2.payload;
        }
      }
...
// src/utils/request.authExpired.test.ts
...
var retryResult = await request_default("/api/react/v1/store-order/sync-missing-orders", {
  method: "POST",
  data: {}
});
...
assertEqual(
  refreshRequestHeaders?.["X-Client-Public-IP"],
  "8.8.8.88",
...
try {
  await request_default("/api/react/v1/store-order/sync-missing-orders", { method: "POST", data: {} });
