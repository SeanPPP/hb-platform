...
async function run() {
  ...
  const readContracts: Array<{
    ...
    },
    {
      ...
      method: "POST",
      path: "/react/v1/posm-sales-orders/list",
      body: {},
      assertData: (data) => assert.ok(data.items.length > 0),
    },
    {
    ...
    },
    {
      ...
      assertData: (data) => assert.equal(data.storeTimeZone, "Australia/Brisbane"),
    },
    {
  ...
  const seasonalCatalog = normalizeSeasonalCardCatalogResponse(
    seasonalCatalogRaw
  );
  ...
  await request(
    "POST",
    "/mobile/device-management/review-device-001/disable"
  );
  assert.equal((await getNormalizedDevices())[0]?.status, 0);
  await request(
    "POST",
    "/mobile/device-management/review-device-001/lock"
  );
  assert.equal((await getNormalizedDevices())[0]?.status, 2);
  await request(
    "POST",
    "/mobile/device-management/review-device-001/activate"
  );
  assert.equal((await getNormalizedDevices())[0]?.status, 1);
  ...
  assert.equal(productDetail.storePrice.uuid, "review-store-price-REV-PROD-001");
  ...
  assert.equal(
    multiCodePage.items.some(
      (item: { uuid: string; barcode: string }) =>
        item.uuid === createdMultiCode.uuid &&
        item.barcode === "9330000000079"
    ),
    true,
  ...
  const createdApproval = pendingApprovals.find(
    (item: { sourceGuid: string }) => item.sourceGuid === createdLeave.leaveGuid
  );
  ...
  assert.equal(
    leaveRequests.find(
      (item: { leaveGuid: string }) => item.leaveGuid === createdLeave.leaveGuid
    )?.status,
    "Approved"
  );
  ...
  const rejectedApproval = pendingApprovals.find(
    (item: { sourceGuid: string }) => item.sourceGuid === rejectedLeave.leaveGuid
  );
  ...
  assert.equal(
    leaveRequests.find(
      (item: { leaveGuid: string }) => item.leaveGuid === rejectedLeave.leaveGuid
    )?.status,
    "Rejected"
  );
  ...
  assert.equal(exportData instanceof ArrayBuffer, true, "导出必须返回二进制数据");
