...
function createInitialState(now = new Date()): AppRouteState {
  ...
  const products: JsonRecord[] = [
    {
      ...
      productImage: "data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%3E%3C/svg%3E",
  ...
  const installmentOrders = [
    {
      orderGuid: "review-installment-001",
      branchCode: "REV001",
      branchName: "Demo Brisbane",
      abn: "12000000000",
      brandName: "Hot Bargain Demo",
      deviceCode: "REV-IOS-01",
      orderNo: "REV-POSM-0001",
      orderTime: timestamp,
...
function createCartLine(product: JsonRecord, quantity: number, timestamp: string) {
  return {
    ...
    importAmount: (product.importPrice ?? product.purchasePrice ?? 0) * quantity,
...
function summarizeCart(cart: JsonRecord): JsonRecord {
  ...
  return {
    ...cart,
    totalAmount: items.reduce((sum: number, item: JsonRecord) => sum + Number(item.amount ?? 0), 0),
    totalQuantity: items.reduce((sum: number, item: JsonRecord) => sum + Number(item.quantity ?? 0), 0),
...
function getHolder(dataStore: ReviewDataStore) {
  ...
  if (!holder) {
    holder = { current: createInitialState() };
...
export function registerIosReviewAppRoutes(
  ...
  register(transport, ["POST"], "/react/v1/store-order/dynamic-data", ({ body }) => {
    ...
    return {
      data: codes.map((productCode: string) => ({
        ...
        lastAllocQuantity: 2,
        cartQuantity:
          cart?.items?.find((item: JsonRecord) => item.productCode === productCode)
            ?.quantity ?? 0,
      })),
    };
  });
  register(transport, ["POST"], "/react/v1/store-order/products/scan-lookup", ({ body }) => {
    const payload = asRecord(body);
    const barcode = String(payload.barcode ?? payload.Barcode ?? "");
    return {
      data: {
        barcode,
        items: state().products.filter((product) => product.barcode === barcode),
      },
    };
  });
  register(transport, ["GET"], /^\/react\/v1\/store-order\/cart\/([^/]+)$/i, ({ match }) => {
    const storeCode = decodeURIComponent(match?.[1] ?? "REV001");
    const current = state();
    const cart = current.carts.get(storeCode) ?? createCart(storeCode, [], current.now);
  ...
  const mutateCart: RouteHandler = ({ path, body }) => {
    ...
    const scannedProduct = current.products.find(
      (item) => item.barcode === payload.barcode || item.barcode === payload.Barcode
    );
    ...
    if (path.endsWith("/clear")) {
      cart.items = [];
      mirrorCreate(current, dataStore, "carts", `cart-clear:${storeCode}`, `Cleared ${storeCode}`, "cleared");
      ...
      if (removeIndex >= 0) {
        ...
        removed = true;
        mirrorRemove(current, dataStore, "carts", `cart:${storeCode}:${removedItem.productCode}`);
      }
    } else {
      ...
      if (existingIndex >= 0) {
        changedItem = {
          ...
          importAmount: Number(cart.items[existingIndex].importPrice) * quantity,
          totalVolume: Number(cart.items[existingIndex].volume ?? 0) * quantity,
          updatedAt: current.now,
        };
        cart.items[existingIndex] = changedItem;
        mirrorUpdate(current, dataStore, "carts", `cart:${storeCode}:${productCode}`, `Cart ${productCode}`, "updated");
      } else {
        changedItem = createCartLine(product, quantity, current.now);
        cart.items.push(changedItem);
        mirrorCreate(current, dataStore, "carts", `cart:${storeCode}:${productCode}`, `Cart ${productCode}`);
      }
    }
  ...
  register(transport, ["POST"], "/react/v1/store-order/submit", ({ body }) => {
    ...
    mirrorCreate(current, dataStore, "orders", orderGUID, order.orderNo, "submitted");
    return { data: clone(order), status: 201 };
  });
  register(transport, ["POST"], "/react/v1/store-order/list", () => ({
    data: page(state().orders),
  }));
  register(transport, ["GET"], /^\/react\/v1\/store-order\/detail\/([^/]+)$/i, ({ match }) => {
  ...
  register(transport, ["GET"], "/react/v1/product-warehouse/mobile/lookup", ({ query }) => {
    const keyword = (query.get("keyword") ?? "").toLowerCase();
    return {
      data: state().products.filter((product) =>
        [product.productCode, product.itemNumber, product.barcode, product.productName]
          .join(" ")
          .toLowerCase()
          .includes(keyword)
      ),
  ...
  register(transport, ["GET", "PATCH"], /^\/react\/v1\/product-warehouse\/mobile\/([^/]+)$/i, ({ method, match, body }) => {
    const productCode = decodeURIComponent(match?.[1] ?? "");
    const product = state().products.find((item) => item.productCode === productCode);
    if (!product) throw new Error(`IOS_REVIEW_PRODUCT_NOT_FOUND: ${productCode}`);
    if (method === "PATCH") {
      Object.assign(product, asRecord(body), { updatedAt: state().now });
      mirrorUpdate(state(), dataStore, "warehouse", `warehouse:${productCode}`, product.productName);
    }
    return { data: clone(product) };
  });
  register(transport, ["PUT"], /^\/react\/v1\/product-warehouse\/mobile\/([^/]+)\/location$/i, ({ match, body }) => {
    const productCode = decodeURIComponent(match?.[1] ?? "");
    const product = state().products.find((item) => item.productCode === productCode);
    if (!product) throw new Error(`IOS_REVIEW_PRODUCT_NOT_FOUND: ${productCode}`);
  ...
  register(transport, ["GET"], /^\/react\/v1\/product-warehouse\/mobile\/([^/]+)\/print-payload$/i, ({ match, query }) => {
    const productCode = decodeURIComponent(match?.[1] ?? "");
    const product = state().products.find((item) => item.productCode === productCode);
    if (!product) throw new Error(`IOS_REVIEW_PRODUCT_NOT_FOUND: ${productCode}`);
    if (query.get("type") === "location") {
      ...
      return { data: clone({ ...location, itemNumber: product.itemNumber, productName: product.productName }) };
    }
    return { data: clone(product) };
  });
  register(transport, ["POST"], /^\/react\/v1\/product-warehouse\/mobile\/[^/]+\/image-upload-signature$/i, () => ({
    data: {
  ...
  register(transport, ["POST"], "/react/v1/product-warehouse/detect", ({ body }) => ({
    data: (Array.isArray(asRecord(body).Items) ? asRecord(body).Items : []).map(
      (item: JsonRecord) => ({
  ...
  register(transport, ["GET"], "/react/v1/locations/mobile/unused", () => ({
    data: { items: clone(state().locations.filter((item) => item.productCount === 0)) },
  }));
  register(transport, ["GET"], /^\/react\/v1\/locations\/([^/]+)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["POST"], "/react/v1/locations", ({ body }) => {
    ...
    mirrorCreate(current, dataStore, "warehouse", location.locationGuid, String(location.locationCode ?? "Demo location"));
    return { data: clone(location), status: 201 };
  });
  register(transport, ["PUT", "DELETE"], /^\/react\/v1\/locations\/([^/]+)$/i, ({ method, match, body }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
    const index = current.locations.findIndex((item) => item.locationGuid === id);
    ...
    mirrorUpdate(current, dataStore, "warehouse", id, String(current.locations[index].locationCode ?? id));
    return { data: clone(current.locations[index]) };
  });
  register(transport, ["POST"], /^\/react\/v1\/locations\/([^/]+)\/products\/bind$/i, ({ match, body }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
    ...
    if (product && !location.products.some((item: JsonRecord) => item.productCode === product.productCode)) {
      location.products.push(clone(product));
      location.productCount = location.products.length;
    }
    mirrorUpdate(current, dataStore, "warehouse", id, String(location.locationCode ?? id));
    return { data: clone(location) };
  });
  register(transport, ["DELETE"], /^\/react\/v1\/locations\/([^/]+)\/products\/([^/]+)$/i, ({ match }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["GET"], /^\/react\/v1\/containers\/([^/]+)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["PUT"], /^\/react\/v1\/containers\/([^/]+)$/i, ({ match, body }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const container = state().containers.find(
      (item) => item.hguid === id || item.HGUID === id
    );
    ...
    mirrorUpdate(state(), dataStore, "warehouse", id, "Updated demo container");
    return { data: { success: true } };
  });
  register(transport, ["POST"], /^\/react\/v1\/containers\/([^/]+)\/products\/query$/i, () => ({
    data: {
      ...
      totalComputed: true,
      statsComputed: true,
      tagStats: { all: state().containerDetails.length, existing: state().containerDetails.length },
    },
  }));
  register(transport, ["POST"], /^\/react\/v1\/containers\/[^/]+\/products\/export$/i, () => ({
    data: binaryExport(),
    headers: {
  ...
  register(transport, ["POST"], /^\/react\/v1\/containers\/(?:batch-update-details|batch-delete-details|sync-from-hq|push-to-hbsales|details\/align-domestic-product-code)$/i, ({ path, body }) => {
    const current = state();
  ...
  register(transport, ["POST"], /^\/react\/v1\/containers\/[^/]+\/actions\/(?:apply-float-rate|apply-prices|recalculate-costs|backfill-last-prices)$/i, () => ({
    data: { success: true, totalRequested: 1, totalUpdated: 1 },
  }));
  register(transport, ["POST"], /^\/react\/v1\/(?:container-products\/create-new-products|container-products\/submit-container|products\/push-to-hq)\/jobs$/i, ({ path, body }) => {
    const current = state();
  ...
  register(transport, ["GET"], /^\/react\/v1\/(?:container-products\/create-new-products|products\/push-to-hq)\/jobs\/([^/]+)$/i, ({ match }) => ({
    data: {
  ...
  register(transport, ["GET"], /^\/v1\/domestic-product-creation\/batch\/([^/]+)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["POST"], "/v1/domestic-product-creation/batch", ({ body }) => {
    const current = state();
    const payload = asRecord(body);
    const batch = {
      ...payload,
      batchNumber: String(payload.batchNumber ?? nextId(current, "REV-BATCH")),
      createdTime: current.now,
      createdBy: "App Review Demo",
      items: Array.isArray(payload.items) ? payload.items : [],
    };
    current.domesticBatches.unshift(batch);
    mirrorCreate(current, dataStore, "domesticPurchase", batch.batchNumber, batch.batchNumber);
    return { data: clone(batch), status: 201 };
  });
  register(transport, ["PUT"], /^\/v1\/domestic-product-creation\/batch\/([^/]+)\/items$/i, ({ match, body }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
    const batch = findByAnyId(current.domesticBatches, id);
    if (!batch) throw new Error(`IOS_REVIEW_BATCH_NOT_FOUND: ${id}`);
    batch.items = Array.isArray(asRecord(body).items) ? asRecord(body).items : body;
    mirrorUpdate(current, dataStore, "domesticPurchase", id, id);
    return { data: clone(batch) };
  });
  register(transport, ["GET"], /^\/v1\/domestic-product-creation\/batch\/[^/]+\/export$/i, () => ({
    data: binaryExport(),
  ...
  register(transport, ["PUT"], /^\/v1\/DomesticProducts\/([^/]+)$/i, ({ match, body }) => {
    const current = state();
    const code = decodeURIComponent(match?.[1] ?? "");
    const product = current.domesticProducts.find((item) => item.productCode === code);
    if (!product) throw new Error(`IOS_REVIEW_PRODUCT_NOT_FOUND: ${code}`);
    Object.assign(product, asRecord(body), { productCode: code });
    mirrorUpdate(current, dataStore, "domesticPurchase", code, product.productName);
    return { data: clone(product) };
  });
  ...
  register(transport, ["POST"], "/react/v1/local-supplier-invoices/grid", () => ({
    data: page(state().invoices),
  }));
  register(transport, ["GET"], /^\/react\/v1\/local-supplier-invoices\/([^/]+)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["POST"], /^\/react\/v1\/local-supplier-invoices\/([^/]+)\/details\/grid$/i, () => ({
  ...
  register(transport, ["POST"], "/react/v1/advertisements/upload-signature", () => ({
    data: {
      url: "data:application/octet-stream;base64,UkVWSUVX",
  ...
  register(transport, ["POST"], "/react/v1/advertisements", ({ body }) => {
    ...
    mirrorCreate(current, dataStore, "advertisements", advertisement.id, String(advertisement.title ?? "Demo ad"));
    return { data: clone(advertisement), status: 201 };
  });
  register(transport, ["GET", "PUT", "DELETE"], /^\/react\/v1\/advertisements\/([^/]+)$/i, ({ method, match, body }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
    const index = current.advertisements.findIndex((item) => item.id === id);
    if (index < 0) throw new Error(`IOS_REVIEW_ADVERTISEMENT_NOT_FOUND: ${id}`);
    if (method === "DELETE") {
    ...
    if (method === "PUT") {
      current.advertisements[index] = { ...current.advertisements[index], ...asRecord(body), id };
      mirrorUpdate(current, dataStore, "advertisements", id, String(current.advertisements[index].title ?? id));
    }
    return { data: clone(current.advertisements[index]) };
  });
  register(transport, ["POST"], /^\/react\/v1\/advertisements\/([^/]+)\/enable$/i, ({ match, query }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const item = state().advertisements.find((candidate) => candidate.id === id);
    if (!item) throw new Error(`IOS_REVIEW_ADVERTISEMENT_NOT_FOUND: ${id}`);
    item.isEnabled = query.get("enable") !== "false";
    mirrorUpdate(state(), dataStore, "advertisements", id, item.title, item.isEnabled ? "active" : "disabled");
    return { data: true };
  });
  ...
  register(transport, ["POST"], "/react/v1/promotions/store", ({ body }) => {
    ...
    mirrorCreate(current, dataStore, "promotions", promotion.id, String(promotion.name ?? "Demo promotion"));
    return { data: clone(promotion), status: 201 };
  });
  register(transport, ["POST"], "/react/v1/promotions/store/copy", ({ body }) => {
    const current = state();
    const payload = asRecord(body);
    const source = current.promotions.find((item) => item.id === payload.sourcePromotionId) ?? current.promotions[0];
    const promotion = {
      ...clone(source),
      id: nextId(current, "review-promotion"),
    ...
    current.promotions.unshift(promotion);
    mirrorCreate(current, dataStore, "promotions", promotion.id, promotion.name);
    return { data: clone(promotion), status: 201 };
  });
  register(transport, ["GET", "PUT"], /^\/react\/v1\/promotions\/store\/([^/]+)$/i, ({ method, match, body }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["POST"], /^\/react\/v1\/promotions\/store\/([^/]+)\/enable$/i, ({ match, query }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const promotion = state().promotions.find((item) => item.id === id);
    if (!promotion) throw new Error(`IOS_REVIEW_PROMOTION_NOT_FOUND: ${id}`);
    promotion.isEnabled = query.get("enable") !== "false";
    mirrorUpdate(state(), dataStore, "promotions", id, promotion.name, promotion.isEnabled ? "active" : "disabled");
    return { data: true };
  });

  register(transport, ["POST"], "/react/v1/store-product-maintenance/lookup", ({ body }) => {
  ...
  register(transport, ["GET"], /^\/react\/v1\/store-product-maintenance\/([^/]+)(?:\/fast-detail)?$/i, ({ match }) => {
    const code = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["GET"], /^\/react\/v1\/store-product-maintenance\/([^/]+)\/codes$/i, ({ match, query }) => {
    const code = decodeURIComponent(match?.[1] ?? "");
    const detail = state().productDetails.get(code);
    if (!detail) throw new Error(`IOS_REVIEW_PRODUCT_NOT_FOUND: ${code}`);
  ...
  register(transport, ["POST"], "/react/v1/products/create-with-prices", ({ body }) => {
    const current = state();
    const payload = asRecord(body);
    const productCode = String(payload.productCode ?? nextId(current, "REV-PROD"));
    ...
    current.productDetails.set(productCode, createProductDetailFixture(product));
    mirrorCreate(current, dataStore, "products", productCode, String(product.productName ?? productCode));
    return { data: { productCode, product: clone(product), created: true }, status: 201 };
  });
  register(transport, ["PUT"], /^\/react\/v1\/store-product-maintenance\/store-prices\/([^/]+)$/i, ({ match, body }) => {
    const current = state();
  ...
  register(transport, ["POST"], /^\/react\/v1\/store-product-maintenance\/store-prices\/([^/]+)\/sync-warehouse$/i, ({ match, body }) => {
    const current = state();
    ...
    mirrorUpdate(current, dataStore, "products", uuid, "Warehouse price sync", "synced");
    return {
      data: {
  ...
  register(transport, ["POST"], "/react/v1/store-product-maintenance/evaluate-auto-pricing", ({ body }) => ({
    data: {
      ...asRecord(body),
  ...
  register(transport, ["PUT"], /^\/react\/v1\/store-product-maintenance\/products\/([^/]+)\/(?:type|clearance-price)$/i, ({ path, match, body }) => {
    const code = decodeURIComponent(match?.[1] ?? "");
    const product = state().products.find((item) => item.productCode === code);
    const detail = state().productDetails.get(code);
    if (!product || !detail) throw new Error(`IOS_REVIEW_PRODUCT_NOT_FOUND: ${code}`);
    ...
    if (path.endsWith("/type")) {
      return { data: { productCode: code, productType: product.productType ?? 1, productTypeLabel: "Standard" } };
    }
    detail.clearancePrice = {
      ...
      storeName: "Demo Brisbane",
      productCode: code,
      clearanceBarcode: asRecord(body).clearanceBarcode ?? IOS_REVIEW_SAMPLE_BARCODE,
      clearancePrice: asRecord(body).clearancePrice ?? 9.95,
    };
    return { data: clone(detail.clearancePrice) };
  });
  register(transport, ["POST"], "/react/v1/store-product-maintenance/set-codes", ({ body }) => {
    const current = state();
    const payload = asRecord(body);
    const detail = current.productDetails.get(String(payload.productCode ?? ""));
    if (!detail) throw new Error(`IOS_REVIEW_PRODUCT_NOT_FOUND: ${payload.productCode ?? ""}`);
  ...
  register(transport, ["PUT", "DELETE"], /^\/react\/v1\/store-product-maintenance\/(set-codes|multi-codes)\/([^/]+)$/i, ({ method, match, body }) => {
    const id = decodeURIComponent(match?.[2] ?? "");
    const detail = Array.from(state().productDetails.values()).find((item) =>
  ...
  register(transport, ["POST"], "/react/v1/posm-sales-orders/list", () => ({
    data: page(state().installmentOrders),
  }));
  register(transport, ["GET"], /^\/react\/v1\/posm-sales-orders\/detail\/([^/]+)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const order = findByAnyId(state().installmentOrders, id);
    if (!order) throw new Error(`IOS_REVIEW_INSTALLMENT_NOT_FOUND: ${id}`);
    return {
      data: {
        order: clone(order),
        orderDetails: clone(order.orderDetails ?? []),
        paymentDetails: clone(order.paymentDetails ?? []),
      },
    };
  });
  ...
  register(transport, ["GET"], /^\/react\/v1\/store-vouchers\/([^/]+)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
  ...
  register(transport, ["GET"], "/react/v1/seasonal-card-remaining/catalog", () => ({
    data: clone(state().seasonalCatalog),
  }));
  register(transport, ["GET"], "/react/v1/seasonal-card-remaining/submissions", () => ({
    data: page(state().seasonalSubmissions),
  }));
  register(transport, ["POST"], "/react/v1/seasonal-card-remaining/submissions", ({ body }) => {
    const current = state();
    const payload = asRecord(body);
    ...
    mirrorCreate(current, dataStore, "seasonalCards", submission.submissionGuid, "Seasonal card submission");
    return { data: clone(submission), status: 201 };
  });
  register(transport, ["GET"], /^\/react\/v1\/seasonal-card-remaining\/submissions\/([^/]+)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const submission = findByAnyId(state().seasonalSubmissions, id);
    if (!submission) throw new Error(`IOS_REVIEW_SEASONAL_SUBMISSION_NOT_FOUND: ${id}`);
...
function registerAttendanceRoutes(
  ...
  register(transport, ["POST"], "/react/v1/attendance/my/availability", ({ body }) => {
    const current = state();
    const payload = asRecord(body);
    const segments = Array.isArray(payload.segments) ? payload.segments : [payload];
    ...
    created.forEach((item) => mirrorCreate(current, dataStore, "attendance", item.availabilityGuid, "Availability"));
    return { data: clone(created), status: 201 };
  });
  register(transport, ["PUT"], /^\/react\/v1\/attendance\/my\/availability\/([^/]+)$/i, ({ match, body }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
    const item = current.availability.find((candidate) => candidate.availabilityGuid === id);
    if (!item) throw new Error(`IOS_REVIEW_AVAILABILITY_NOT_FOUND: ${id}`);
    Object.assign(item, asRecord(body), {
      workDate: asRecord(body).availableDate ?? asRecord(body).workDate ?? item.workDate,
  ...
  register(transport, ["POST"], /^\/react\/v1\/attendance\/my\/availability\/([^/]+)\/cancel$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const item = state().availability.find((candidate) => candidate.availabilityGuid === id);
    if (item) item.status = "Cancelled";
    mirrorUpdate(state(), dataStore, "attendance", id, "Availability", "cancelled");
    return { data: { success: true } };
  });
  register(transport, ["POST"], "/react/v1/attendance/punch", ({ body }) => {
    ...
    mirrorCreate(current, dataStore, "attendance", punch.punchGuid, `Punch ${punch.punchType}`);
    return { data: punch, status: 201 };
  });
  register(transport, ["POST"], "/react/v1/attendance/location-samples", () => ({
    data: { success: true, simulated: true },
  }));
  register(transport, ["GET"], "/react/v1/attendance/my/leave-requests", () => ({
    data: clone(state().leaveRequests),
  }));
  register(transport, ["POST"], /^\/react\/v1\/attendance\/(?:my|managed)\/leave-requests$/i, ({ body }) => {
    const current = state();
    ...
    mirrorCreate(current, dataStore, "attendance", leave.leaveGuid, "Leave request", "pending");
    return { data: clone(leave), status: 201 };
  });
  register(transport, ["POST"], /^\/react\/v1\/attendance\/my\/leave-requests\/([^/]+)\/cancel$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const leave = state().leaveRequests.find((item) => item.leaveGuid === id);
    if (leave) leave.status = "Cancelled";
    mirrorUpdate(state(), dataStore, "attendance", id, "Leave request", "cancelled");
    return { data: { success: true } };
  });
  register(transport, ["POST"], "/react/v1/attendance/leave-attachments/upload-signature", () => ({
    data: {
      url: "data:application/octet-stream;base64,UkVWSUVX",
  ...
  register(transport, ["GET"], "/react/v1/attendance/approvals/pending", () => ({
    data: clone(state().approvals.filter((item) => item.status === "Pending")),
  }));
  register(transport, ["POST"], /^\/react\/v1\/attendance\/approvals\/([^/]+)\/(approve|reject)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    const decision = match?.[2] === "approve" ? "Approved" : "Rejected";
    const approval = state().approvals.find((item) => item.approvalGuid === id);
    if (approval) {
    ...
    mirrorUpdate(state(), dataStore, "attendance", id, "Attendance approval", decision.toLowerCase());
    return { data: { success: true } };
  });
  register(transport, ["GET"], "/react/v1/attendance/schedules/week", () => ({
    data: clone(state().schedules),
  }));
  register(transport, ["POST"], "/react/v1/attendance/schedules", ({ body }) => {
    const current = state();
    const schedule = {
      ...asRecord(body),
    ...
    mirrorCreate(current, dataStore, "attendance", schedule.scheduleGuid, "Attendance schedule", schedule.status);
    return { data: clone(schedule), status: 201 };
  });
  register(transport, ["PUT", "DELETE"], /^\/react\/v1\/attendance\/schedules\/([^/]+)$/i, ({ method, match, body }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
    const index = current.schedules.findIndex((item) => item.scheduleGuid === id);
    ...
    current.schedules[index] = { ...current.schedules[index], ...asRecord(body), scheduleGuid: id };
    mirrorUpdate(current, dataStore, "attendance", id, "Attendance schedule", current.schedules[index].status);
    return { data: clone(current.schedules[index]) };
  });
  register(transport, ["POST"], "/react/v1/attendance/schedules/publish-week", () => ({
  ...
  register(transport, ["POST"], "/react/v1/attendance/holidays/sync", ({ body }) => ({
    data: {
      ...asRecord(body),
  ...
  register(transport, ["POST"], "/react/v1/attendance/holidays", ({ body }) => {
    ...
    mirrorCreate(current, dataStore, "attendance", holiday.holidayGuid, String(holiday.holidayName ?? "Holiday"));
    return { data: clone(holiday), status: 201 };
  });
  register(transport, ["PUT", "DELETE"], /^\/react\/v1\/attendance\/holidays\/([^/]+)$/i, ({ method, match, body }) => {
    const current = state();
    const id = decodeURIComponent(match?.[1] ?? "");
    const index = current.holidays.findIndex((item) => item.holidayGuid === id);
    ...
    current.holidays[index] = { ...current.holidays[index], ...asRecord(body), holidayGuid: id };
    mirrorUpdate(current, dataStore, "attendance", id, String(current.holidays[index].holidayName ?? "Holiday"));
    return { data: clone(current.holidays[index]) };
  });
  register(transport, ["GET"], /^\/react\/v1\/attendance\/employees\/[^/]+\/week$/i, () => ({
    data: {
      weekStart: state().today,
      weekEnd: state().today,
      days: [{ workDate: state().today, dayOfWeek: 1, schedules: clone(state().schedules) }],
    },
  }));
  register(transport, ["GET"], /^\/react\/v1\/attendance\/employees\/[^/]+\/records$/i, () => ({
    data: {
...
function registerDeviceRoutes(
  ...
  register(transport, ["POST"], /^\/mobile\/device-management\/([^/]+)\/(activate|disable|lock)$/i, ({ match }) => {
    const id = decodeURIComponent(match?.[1] ?? "");
    ...
    mirrorUpdate(state(), dataStore, "devices", id, String(device.deviceName ?? id), device.status);
    return { data: { success: true } };
  });
  ...
  register(transport, ["GET"], "/mobile/app-device-status/summary", () => ({
    data: { total: 1, online: 1, offline: 0, android: 0, ios: 1, unknownSystem: 0 },
  }));
...
function registerReportRoutes(
  ...
  const state = () => holder.current;
  register(transport, ["GET"], "/react/v1/product-movement-report/store-options", () => ({
    data: IOS_REVIEW_STORES.map((store) => ({
      storeCode: store.storeCode,
      storeName: store.storeName,
    })),
  }));
  register(transport, ["GET"], "/react/v1/dashboard/executive-branch-performance", () => ({
    data: {
  ...
  register(transport, ["GET"], "/react/v1/dashboard/executive-hourly-traffic", () => ({
    data: {
  ...
  register(transport, ["GET"], "/react/v1/dashboard/branch-daily-performance", () => ({
    data: {
      items: IOS_REVIEW_STORES.map((store, index) => ({
        date: state().today,
        branchCode: store.storeCode,
  ...
  register(transport, ["GET"], /^\/react\/v1\/dashboard\/(?:china-supplier-sales-rank|supplier-sales-rank)$/i, () => ({
      data: {
        items: [
          {
            supplierCode: "REV-SUP-001",
  ...
  register(transport, ["GET"], "/react/v1/dashboard/enhanced-sales-product-details", () => ({
    data: page(
  ...
  register(transport, ["GET"], /^\/react\/v1\/dashboard\/(?:china-supplier-store-sales|supplier-store-sales)$/i, () => ({
    data: {
      items: IOS_REVIEW_STORES.map((store, index) => ({
        branchCode: store.storeCode,
        branchName: store.storeName,
  ...
  register(transport, ["GET"], "/react/v1/dashboard/product-sales-by-branches", () => ({
    data: {
      items: IOS_REVIEW_STORES.map((store, index) => ({
        branchCode: store.storeCode,
        branchName: store.storeName,
  ...
  register(transport, ["GET"], "/react/v1/dashboard/statistics-freshness", () => ({
    data: {
