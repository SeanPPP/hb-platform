...
const sensitiveRequestKey = ["employee-profile", "sensitive-change-request", "me", "user-a"] as const;
...
queryClient.setQueryData(sensitiveRequestKey, { bankAccountNumber: "pending-secret-account" });
...
assert.equal(queryClient.getQueryData(sensitiveRequestKey), undefined, "会话清理必须移除待审敏感资料缓存");
